...
| 장애물 배치 | Obstacle 선택 후 클릭=중심, 누른 채 드래그=회전각, 떼면 배치 (크기 = `obstacle w` / `obstacle h` 슬라이더) |
...
중앙 2m x 2m 시험 영역, 로봇 자세, 회전 직사각형 장애물의 중심/크기/각도/꼭짓점,
색상 목표 위치, 센서 스냅샷, 현재 판정 상태, 그리고 실패 가능 원인을 함께 담는다.
...
3. **카메라 가림(occlusion) 미모델** — FOV+사거리 안의 색종이는 장애물 뒤라도 보인다.
4. **라이다**는 합성 스캔이라 캘리브 상수(`ANGLE_OFFSET_DEG` 등)의 *효과*(필터·전방
   선별·서브샘플)는 검증되지만, 상수의 *실차 정확도*는 검증 대상이 아니다.
5. **main() 의 제어 루프 글루**는 `sim.py` 의 `brain_tick()` 이 미러링한다.
