...
# [FIX 1] Response header validation: added check for data type byte (header[6] != 0x81)
# Original code only checked header[0] and header[1], missing the scan data type confirmation.
class RPLidarC1:
    def __init__(self, port, baud):
        ...
        if len(header) != 7 or header[0] != 0xA5 or header[1] != 0x5A or header[6] != 0x81:
            self.ser.close()
    ...
    # [FIX 2] Packet sync: read 1 byte at a time to find valid start byte,
    # then read remaining 4. Original read(5) on sync failure skipped a full
    # 5-byte block, causing misalignment with the actual packet boundaries.
    def _loop(self):
        buf_a, buf_d, buf_q = [], [], []
        ...
        while self.running:
            try:
                ...
                s_inv = (b[0] & 0x02) >> 1
                if s_inv != (1 - s_flag):
                    continue  # Not a valid start byte; advance by 1 byte to re-sync
...
# [FIX 5] left_dist now uses np.abs for consistency with right_dist.
# Original: np.min(left_points[:, 1]) — functionally same since y>0 for left_points,
# but np.abs makes intent explicit and symmetric with right side calculation.
def side_obstacle_distances(points):
    ...
    if len(left_points) > 0:
        left_dist = float(np.min(np.abs(left_points[:, 1])))
...
# [FIX 4] Safety bubble applied to ALL detected obstacles, not only the single closest.
# Original used np.argmin to find one obstacle and expanded only that bin,
# leaving nearby obstacles unprotected.
def apply_safety_bubble(ranges, counts):
    ...
    obstacle_indices = np.where(valid_obstacles)[0]
    for idx in obstacle_indices:
        obs_dist = float(ranges[idx])
        half_angle = math.atan2(AVOID_BUBBLE_RADIUS, max(obs_dist, LIDAR_MIN_DIST_M))
        half_bins = int(math.ceil(math.degrees(half_angle) / AVOID_ANGLE_STEP_DEG))
        start = max(0, idx - half_bins)
        end = min(len(working), idx + half_bins + 1)
...
# [FIX 6] Filter initialization changed from first-frame raw value to LIDAR_MAX_DIST_M.
# Original skipped the filter on the first call (direct assignment), so a bad first
# reading would poison the filter state. Now the filter is always applied; initializing
# to LIDAR_MAX_DIST_M (safe/clear) means it converges toward actual values over time.
def smooth_lidar_distances(front_dist, left_dist, right_dist):
    global _filtered_front_dist, _filtered_left_dist, _filtered_right_dist

    if _filtered_front_dist is None:
        _filtered_front_dist = LIDAR_MAX_DIST_M
        _filtered_left_dist = LIDAR_MAX_DIST_M
        _filtered_right_dist = LIDAR_MAX_DIST_M
