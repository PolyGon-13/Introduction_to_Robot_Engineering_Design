...
import restore_square as square
from lidar import (
    ANG_MAX,
    ANG_MIN,
    COLOR_TO_LIDAR_DEG,
...
def make_square_args():
    args = SimpleNamespace(
        square_size=square.SQUARE_SIZE_M,
        homography_file=str(square.DEFAULT_HOMOGRAPHY_FILE),
        camera_matrix_file=str(square.DEFAULT_CAMERA_MATRIX_FILE),
        dist_coeffs_file=str(square.DEFAULT_DIST_COEFFS_FILE),
        min_area=square.MIN_AREA,
        min_ground_span=square.MIN_GROUND_SPAN_M,
        max_ground_span=square.MAX_GROUND_SPAN_M,
        complete_fill_ratio=square.COMPLETE_FILL_RATIO,
        complete_size_min_ratio=square.COMPLETE_SIZE_MIN_RATIO,
        complete_size_max_ratio=square.COMPLETE_SIZE_MAX_RATIO,
        max_projected_area_ratio=square.MAX_PROJECTED_AREA_RATIO,
        center_margin=square.CENTER_MARGIN_RATIO,
        border_touch_px=square.BORDER_TOUCH_PX,
        out_of_frame_px=square.OUT_OF_FRAME_PX,
        lidar_obstacle_d=0.60,
        lidar_half_deg=square.LIDAR_GUIDE_HALF_DEG,
        min_restore_shift=square.MIN_RESTORE_SHIFT_M,
        max_restore_shift=square.MAX_RESTORE_SHIFT_M,
        smooth=0.65,
    )
...
def load_square_geometry(args):
    camera_matrix, dist_coeffs = square.load_tuned_intrinsics(args)
    if camera_matrix is None or dist_coeffs is None:
        raise RuntimeError(
            f"[CALIB] intrinsics not found:\n  {args.camera_matrix_file}\n  {args.dist_coeffs_file}"
        )

    path = square.homography_path(args)
    if not path.exists():
        raise RuntimeError(f"[CALIB] homography not found: {path}")

    undistorter = square.Undistorter(
        camera_matrix, dist_coeffs, square.CALIB_WIDTH, square.CALIB_HEIGHT
    )
    projector = square.GroundProjector(square.load_homography(path))
    print(f"[CALIB] loaded homography: {path}")
    return undistorter, projector


def square_track_target(target, selected, projector, frame_shape):
    if target is None or selected is None:
        return None

    center = square.candidate_display_center(selected, projector, frame_shape)
    if center is None:
        return None
...
def main():
    ...
    square_args = make_square_args()
    undistorter, projector = load_square_geometry(square_args)
        ...
        while True:
            ...
            if color_in_forward_zone:
                ...
                mode, target_v, target_w = avoid_mode(
                    ...
                    "AVOID_SEARCH",
                    ranges,
                    last_color_deg if last_color_time > 0.0 else None,
                    elapsed,
                )
