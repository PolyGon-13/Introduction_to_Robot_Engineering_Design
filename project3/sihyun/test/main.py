...
DRIVE_V = 1.0  # 색 추적, 장애물 회피, 색 완료 후 추가 전진에 공통으로 쓰는 전진 속도
V_STEP = 1.0  # 전진 속도 명령의 루프당 최대 변화량
FOLLOW_W_STEP = 11.0  # 색 추적 모드 회전 속도 명령의 루프당 최대 변화량
AVOID_W_STEP = 11.0  # 장애물 회피 모드 회전 속도 명령의 루프당 최대 변화량
LOOP_DT = 0.05  # 메인 루프 대기 시간(초)
SEARCH_MAX_W = 11.0  # 색 재탐색 모드 최대 회전 속도
SEARCH_TURN_GAIN = 11.0  # 마지막 색 방향을 회전 속도로 바꾸는 비례 계수
SWITCH_SEARCH_W = 11.0  # 다음 색이 안 보일 때 제자리 탐색 회전 속도
...
class Motor:
    ...
    def vw(self, v, w):
        with self.write_lock:
            self.ser.write(f"V{1.0:.3f},{11.0:.3f}\n".encode("ascii"))
