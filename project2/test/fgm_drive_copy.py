...
SIDE_GAP_BLOCK_ANGLE_RAD = math.radians(SIDE_GAP_BLOCK_ANGLE_DEG)
CUM_TURN_INCREASE_EPS_RAD = math.radians(1.0)
CUM_TURN_HARD_SPAN_RAD = math.radians(10.0)
CUM_TURN_SOFT_SPAN_RAD = max(1e-6, CUM_TURN_HARD_LIMIT_RAD - CUM_TURN_SOFT_LIMIT_RAD)
...
def filter_gaps_by_width(gaps):
    return [(start, end) for start, end in gaps if end - start >= FGM_MIN_GAP_BINS]
...
def side_gap_penalty(angle_rad, left_dist, right_dist):
    ...
    straight_mask = np.abs(angle_rad) <= SIDE_GAP_BLOCK_ANGLE_RAD
    ...
    if left_dist < SIDE_GAP_WARN_DIST:
        ...
        left_side = angle_rad > SIDE_GAP_BLOCK_ANGLE_RAD
        penalty += np.where(
            left_side,
    ...
    if right_dist < SIDE_GAP_WARN_DIST:
        ...
        right_side = angle_rad < -SIDE_GAP_BLOCK_ANGLE_RAD
        penalty += np.where(
            right_side,
            SIDE_GAP_PENALTY_WEIGHT * pressure,
            0.0,
...
def cumulative_turn_penalty(angle_rad, accumulated_turn_rad):
    ...
    increasing_turn = abs_projected > (abs_accumulated + CUM_TURN_INCREASE_EPS_RAD)
    ...
    soft_penalty = (
        CUM_TURN_SOFT_PENALTY_WEIGHT
        * (soft_excess / CUM_TURN_SOFT_SPAN_RAD) ** 2
    )
    hard_penalty = CUM_TURN_HARD_PENALTY_WEIGHT * (
        hard_excess / CUM_TURN_HARD_SPAN_RAD + (hard_excess > 0.0)
    )
...
def choose_target_from_gaps(
    ...
    if not gaps:
        return -1, (0, 0), -float("inf"), 0.0, accumulated_turn_rad, 0.0, False
    ...
    for start, end in gaps:
        ...
        side_penalty, side_block = side_gap_penalty(
            angle_rad,
            left_dist,
            right_dist,
        )
        scores = scores - turn_penalty - side_penalty
...
def choose_fallback_target(
    ...
    if not usable.any():
        return len(ranges) // 2, 0.0, False
    angle_rad = FGM_ANGLES_RAD
    ...
    usable_ranges = usable_ranges - 0.25 * turn_penalty - 0.25 * side_penalty
    fallback_idx = int(np.argmax(usable_ranges))
    return fallback_idx, float(side_penalty[fallback_idx]), bool(side_block[fallback_idx])
...
def choose_fgm_cmd(scan, prev_w, prev_target_angle, pose, accumulated_turn_rad=0.0):
    ...
    if target_idx < 0:
        target_idx, side_penalty, side_block = choose_fallback_target(
            angles_deg,
            smooth_ranges,
            accumulated_turn_rad,
            info_left,
