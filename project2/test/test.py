...
def apply_safety_bubble(ranges, counts):
    """
    [수정] 기존: 가장 가까운 점 하나에만 버블 적용
    [변경] 모든 장애물 점에 버블 적용
    → 대각선 박스처럼 여러 점이 분산된 경우에도
      모서리 사이 좁은 틈이 gap으로 잡히는 문제 방지
    """
    working = ranges.copy()
    valid = counts > 0
    ...
    # 모든 장애물 점에 버블 적용
    for i in np.where(valid_obstacles)[0]:
        dist = float(ranges[i])
        half_angle_rad = math.atan2(FGM_BUBBLE_RADIUS, max(dist, MIN_LIDAR_DIST_M))
        half_bins = int(math.ceil(math.degrees(half_angle_rad) / FGM_ANGLE_STEP_DEG))
        start = max(0, i - half_bins)
        end = min(len(working), i + half_bins + 1)
