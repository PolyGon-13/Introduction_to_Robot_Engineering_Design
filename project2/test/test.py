...
def arduino_writer():
    ...
    while True:
        ...
        # 엔터 누르기 전에는 Arduino로 명령 전송하지 않음
        if not send_enable:
            continue
...
while True:
    ...
    desired_angle = Follow_the_Gap_Method(distance_array, threshold=600)
    ...
    print(
        f"[DETECT] Angle: {target_angle:.2f}, "
        f"phi_l: {phi_l:.2f}mm, phi_r: {phi_r:.2f} mm"
    )
