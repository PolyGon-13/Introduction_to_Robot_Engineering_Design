...
PREDICT_TIME = 1.50
PREDICT_DT = 0.10
ROBOT_RADIUS = 0.14 # 로봇 반경
SAFETY_MARGIN = 0.14 # 안전 여유
COLLISION_DIST = SAFETY_MARGIN # 충돌 판정 거리
CLEARANCE_CAP = 1.0
FRONT_CORRIDOR_HALF = ROBOT_RADIUS + SAFETY_MARGIN + 0.30 # 정면 통로 반폭
...
def front_distance(points):
    ...
    mask = ((points[:, 0] > 0.0) & # x가 양수 -> 로봇 앞쪽
            (points[:, 0] < ACTIVE_FRONT_DIST + ROBOT_RADIUS) & # x가 특정값 이내 -> 너무 멀지 않음
            (np.abs(points[:, 1]) < FRONT_CORRIDOR_HALF)) # |y|가 특정값 이내 -> 정면 범위 안
    if not mask.any():
        return MAX_LIDAR_DIST_M
    
    # 가장 가까운 정면 장애물과의 거리 (로봇 외곽 기준)
    return float(max(0.0, np.min(points[mask, 0]) - ROBOT_RADIUS))
...
def trajectory_clearances(traj, points):
    ...
    dist = np.maximum(dist - ROBOT_RADIUS, 0.0)
    ...
    front_mask = ((rel_x > 0.0) &
                  (rel_x < ACTIVE_FRONT_DIST + ROBOT_RADIUS) &
...
def choose_best_cmd(scan, prev_w, cmd_v):
    ...
    if sb.any():
        ...
        ry = ys_sb[ys_sb < -0.05]
        if len(ly) > 0:
            info_left = max(0.0, float(np.min(ly)) - ROBOT_RADIUS)
        if len(ry) > 0:
