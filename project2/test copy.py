...
# 폭 인식 추가/수정 부분
# =========================================================
# 로봇 폭 20cm + 여유 4cm = 최소 통과 폭 24cm
PASSAGE_MARGIN = 0.04
MIN_PASSAGE_WIDTH = 2.0 * ROBOT_RADIUS + PASSAGE_MARGIN

# 폭 검사 구간
# 너무 가까운 x=0.10 근처 점은 노이즈/모서리 영향이 커서 0.18m부터 검사
PASSAGE_CHECK_X_MIN = 0.18
PASSAGE_CHECK_X_MAX = 0.55
PASSAGE_CHECK_Y_MAX = 0.70

# 중심선 근처 점은 좌/우 벽으로 보지 않음
# 로봇 반폭이 10cm이므로 ±10cm 안쪽 점은 폭 계산에서 제외
PASSAGE_SIDE_MIN_Y = 0.10

# 양쪽 장애물을 이 거리 안에서 발견했을 때만 폭 계산에 사용
SIDE_DETECTED_MAX = 0.80

# 한쪽 벽으로 인정하려면 최소 점 개수
PASSAGE_MIN_SIDE_POINTS = 2

# 폭 부족 시 회피 회전 속도
WIDTH_ESCAPE_W = 0.30

# 폭 부족 회피 방향 유지 시간
WIDTH_ESCAPE_HOLD_S = 1.20
...
def compute_passage_width(points, fallback_left, fallback_right):
    """
    전방 통로 폭 계산.

    기존 문제:
    - min()으로 가장 작은 y값을 잡으면 중심선 근처 노이즈/모서리 점 때문에
      pL=0.08, pR=0.09처럼 너무 좁게 계산될 수 있음.

    수정:
    - PASSAGE_SIDE_MIN_Y = 0.10으로 중심선 근처 점 제외
    - min() 대신 percentile 사용
    - 한쪽당 최소 점 개수 조건 적용
    """

    passage_left = fallback_left
    passage_right = fallback_right

    left_seen = fallback_left < SIDE_DETECTED_MAX
    right_seen = fallback_right < SIDE_DETECTED_MAX

    if len(points) > 0:
        passage_band = (
            (points[:, 0] >= PASSAGE_CHECK_X_MIN)
            & (points[:, 0] <= PASSAGE_CHECK_X_MAX)
            & (np.abs(points[:, 1]) <= PASSAGE_CHECK_Y_MAX)
        )

        if passage_band.any():
            ys = points[passage_band, 1]

            left_points = ys[ys > PASSAGE_SIDE_MIN_Y]
            right_points = ys[ys < -PASSAGE_SIDE_MIN_Y]

            if len(left_points) >= PASSAGE_MIN_SIDE_POINTS:
                measured_left = float(np.percentile(left_points, 20))
                if left_seen:
                    passage_left = min(passage_left, measured_left)
                else:
                    passage_left = measured_left
                left_seen = True

            if len(right_points) >= PASSAGE_MIN_SIDE_POINTS:
                measured_right = float(-np.percentile(right_points, 80))
                if right_seen:
                    passage_right = min(passage_right, measured_right)
                else:
                    passage_right = measured_right
                right_seen = True

    if left_seen and right_seen:
        passage_width = passage_left + passage_right
        too_narrow = passage_width < MIN_PASSAGE_WIDTH
    else:
        passage_width = 999.0
        too_narrow = False

    return too_narrow, passage_width, passage_left, passage_right, left_seen, right_seen
...
def choose_target_from_gaps(
    ...
    for start, end in gaps:
        ...
        straight_score = 1.0 - np.clip(
            np.abs(angle_rad) / max_target_angle,
            0.0,
            1.0,
        )
...
def choose_fgm_cmd(scan, prev_w, prev_target_angle, pose, width_escape_dir):
    ...
    too_narrow, passage_width, passage_left, passage_right, left_seen, right_seen = compute_passage_width(
        points,
        info_left,
        info_right,
    )
    ...
    width_block = too_narrow
    new_width_escape_dir = width_escape_dir

    if width_block:
        v = 0.0

        if new_width_escape_dir == 0:
            if passage_left > passage_right:
                new_width_escape_dir = 1
            else:
                new_width_escape_dir = -1

        escape_w = WIDTH_ESCAPE_W * new_width_escape_dir

        raw_w = float(np.clip(escape_w, -MAX_ABS_W, MAX_ABS_W))
        w = rate_limit_w(prev_w, raw_w, urgent=True)

    else:
        new_width_escape_dir = 0
    ...
    return v, w, target_angle, {
        ...
        "width_escape_dir": new_width_escape_dir,
        "points": len(points),
        "gaps": len(gaps),
        "gap_width": gap_width,
        "gap_right": gap_right,
...
def main():
    ...
    last_width_escape_dir = 0
    last_width_escape_time = 0.0
    ...
    try:
        while True:
            ...
            now_escape = time.time()

            if (
                last_width_escape_dir != 0
                and now_escape - last_width_escape_time > WIDTH_ESCAPE_HOLD_S
            ):
                last_width_escape_dir = 0
            ...
            if info["width_escape_dir"] != 0:
                if last_width_escape_dir == 0:
                    last_width_escape_time = time.time()
                last_width_escape_dir = info["width_escape_dir"]
            else:
                last_width_escape_dir = 0
            ...
            if time.time() - last_log > 0.25:
                print(
                    ...
                    f"minpw={info['min_passage_width']:.2f} "
                    f"narrow={int(info['too_narrow'])} "
                    f"wb={int(info['width_block'])} "
                    f"wdir={info['width_escape_dir']}"
                )
