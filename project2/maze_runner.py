...
PREV_HEADING_COST_URGENT = 0.50
CLEARANCE_COST_URGENT    = 5.00
SWITCH_SIGN_COST_URGENT  = 3.00
SMALL_HEADING_PENALTY_DEG  = 16.0
SMALL_HEADING_PENALTY_GAIN = 5.0
...
def score_pass_heading(heading, clear, front, prev_heading, urgency):
    ...
    if urgency > 0.5:
        commit_ramp = (urgency - 0.5) * 2.0
        cost += SMALL_HEADING_PENALTY_GAIN * commit_ramp * max(
            0.0, np.deg2rad(SMALL_HEADING_PENALTY_DEG) - abs(heading))
    return cost
