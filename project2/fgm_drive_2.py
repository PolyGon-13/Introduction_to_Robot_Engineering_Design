...
FGM_OBSTACLE_INFLATION_RADIUS = ROBOT_RADIUS / 2 + 0.02 # 선분 장애물 확장 반경
FGM_SEGMENT_CONNECT_DIST = 0.05 # 이 거리 이하의 인접점만 선분 연결
FGM_FREE_DIST = ROBOT_RADIUS + 0.03 # 이 이상 뚫려 있어야 지나갈 수 있는 칸으로 인식
...
WALL_ANGLE_HALF_WIDTH_DEG = 3.0 # Wall을 90도 하나가 아니라 +-1도 간격으로 더 확인
...
def apply_safety_bubble(ranges, counts):
    ...
    angles_deg = (
        FGM_MIN_ANGLE_DEG
        + np.arange(len(ranges), dtype=np.float32) * FGM_ANGLE_STEP_DEG
    )
    angles_rad = np.deg2rad(angles_deg)
    ray_cos = np.cos(angles_rad)
    ray_sin = np.sin(angles_rad)

    point_x = ranges * ray_cos
    point_y = ranges * ray_sin
    valid_idxs = np.flatnonzero(valid_obstacles)
    inflated = working.copy()
    affected = np.zeros(len(working), dtype=bool)

    inflation_radius = FGM_OBSTACLE_INFLATION_RADIUS
    connect_dist = max(0.0, FGM_SEGMENT_CONNECT_DIST)

    def point_inflation_entry(px, py):
        projection = px * ray_cos + py * ray_sin
        perpendicular = np.abs(ray_cos * py - ray_sin * px)
        inside = (projection > 0.0) & (perpendicular <= inflation_radius)
        offset = np.sqrt(np.maximum(0.0, inflation_radius * inflation_radius - perpendicular * perpendicular))
        return np.where(inside, np.maximum(0.0, projection - offset), np.inf)

    for idx_a, idx_b in zip(valid_idxs[:-1], valid_idxs[1:]):
        if idx_b != idx_a + 1:
            continue

        ax = float(point_x[idx_a])
        ay = float(point_y[idx_a])
        bx = float(point_x[idx_b])
        by = float(point_y[idx_b])

        segment_dx = bx - ax
        segment_dy = by - ay
        segment_len = math.hypot(segment_dx, segment_dy)
        if segment_len > connect_dist:
            continue

        segment_entry = np.minimum(
            point_inflation_entry(ax, ay),
            point_inflation_entry(bx, by),
        )

        denom = ray_cos * segment_dy - ray_sin * segment_dx
        not_parallel = np.abs(denom) > 1e-6
        ray_t = (ax * segment_dy - ay * segment_dx) / np.where(not_parallel, denom, 1.0)
        segment_t = (ax * ray_sin - ay * ray_cos) / np.where(not_parallel, denom, 1.0)
        intersects = (
            not_parallel
            & (ray_t >= 0.0)
            & (segment_t >= 0.0)
        ...
        segment_entry = np.minimum(
            segment_entry,
            np.where(intersects, np.maximum(0.0, ray_t - inflation_radius), np.inf),
        )

        affected |= np.isfinite(segment_entry)
        inflated = np.minimum(inflated, segment_entry)

    return inflated, closest_idx, closest_dist, int(np.count_nonzero(affected))
