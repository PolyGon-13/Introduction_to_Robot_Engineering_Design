...
const float COUNT_PER_M_CAL = 1.0000f;
...
void setup()
{
  ...
  Serial.println(F("[READY] straight drive (position PID)"));
  Serial.print(F("COUNT_PER_M = "));
  Serial.println(COUNT_PER_M, 3);
  Serial.println(F("Cmd: 'D <m>' or 'S'"));
}
...
void loop()
{
  ...
  if (e_pos <= (float)STOP_TOL_CNT)
  {
    ...
    float dist_avg = 0.5f * (progR + progL) / COUNT_PER_M;
    char buf[64];
    snprintf(buf, sizeof(buf), "DONE %ld %ld %.4f", progR, progL, dist_avg);
    Serial1.println(buf);
    Serial.print(F("[DONE] d_avg="));
    Serial.println(dist_avg, 4);
    return;
  }
  ...
  float V_sync = constrain(kp_sync * e_sync + ki_sync * inte_sync + kd_sync * d_sync,
                           -V_SYNC_LIMIT, V_SYNC_LIMIT);
  e_sync_prev = e_sync;

  writeDriver_r(constrain(V_base - V_sync * driveSign, V_MIN, V_MAX));
  writeDriver_l(constrain(V_base + V_sync * driveSign, V_MIN, V_MAX));
